...
// ============================================================
// ENHANCED HUMANIZER v2 - Beats StealthWriter & GPTZero
// Combines structural transformations with vocabulary changes
// ============================================================
...
const WORD_SWAPS = [
  ...
  { f: /\bincreasingly\b/gi, r: () => pick(['more and more', 'increasingly']) },
  { f: /\bcomplexities\b/gi, r: () => pick(['challenges', 'difficulties']) },
  { f: /\bimplications\b/gi, r: () => pick(['effects', 'consequences']) },
  { f: /\boverlooked\b/gi, r: () => pick(['ignored', 'missed']) },
  { f: /\bspark debate\b/gi, r: () => pick(['cause debate', 'start debate']) },
  { f: /\bprioritize\b/gi, r: () => pick(['put first', 'focus on']) },
  { f: /\bstrike a balance\b/gi, r: () => pick(['find a balance', 'balance']) },
  { f: /\binterconnected\b/gi, r: () => pick(['connected', 'linked']) },
  { f: /\bdeployment\b/gi, r: () => pick(['use', 'rollout']) },
  { f: /\bintegration\b/gi, r: () => pick(['use', 'combination']) },
  { f: /\balgorithms\b/gi, r: () => pick(['systems', 'methods']) },
  { f: /\bplatforms\b/gi, r: () => pick(['tools', 'systems']) },
  { f: /\boperational costs\b/gi, r: () => pick(['costs', 'expenses']) },
  { f: /\bautomated\b/gi, r: () => pick(['automatic', 'self-running']) },
  { f: /\btransparent\b/gi, r: () => pick(['open', 'clear']) },
  { f: /\bwelfare\b/gi, r: () => pick(['well-being', 'wellbeing']) },
  { f: /\badvancement\b/gi, r: () => pick(['progress', 'advances']) },
  { f: /\bresponsibility\b/gi, r: () => pick(['duty', 'accountability']) },
...
// ============================================================
// PHASE 8: Add natural imperfections (DISABLED - causes issues)
// ============================================================
...
const MODES = {
  ...
  aggressive:{ split: true, combine: true, fillers: true, words: true, reorder: true, fakeDepth: true, contractions: true, imperfections: false, reactions: true },
}
...
export function humanize(text, strength = 'medium') {
  ...
  // Phase 9: Imperfections (disabled)
