...
// ============================================================
// PHASE 1: Sentence rewriting for burstiness
// Rewrite each sentence to vary its structure and length
// ============================================================
function rewriteForBurstiness(sentences, strength) {
  const out = []
  const aggressive = strength === 'aggressive'

  for (const sent of sentences) {
    const words = sent.split(/\s+/)

    // Long sentences (18+ words): split or heavily rewrite
    if (words.length >= 18) {
      // Try to find split points
      const split = trySplitSentence(sent)
      if (split) {
        out.push(...split)
        continue
      }
      // If can't split, keep as-is but it will get vocab treatment
    }
...
function trySplitSentence(sent) {
  // Split at semicolons
  if (sent.includes(';')) {
    ...
    return parts.map((p, i) => {
      let s = p.trim()
      if (!s) return null
      if (i > 0 && /^[a-z]/.test(s)) s = s.charAt(0).toUpperCase() + s.slice(1)
      if (!s.match(/[.!?]$/)) s += '.'
      return s
    }).filter(Boolean)
  }

  // Split at ", but " where second part starts with capital
  let m = sent.match(/^(.{15,}?),\s+but\s+([A-Z][a-zA-Z\s',-]{5,})$/)
  if (m) return buildSplit(m[1], m[2])
  ...
  if (m) return buildSplit(m[1], m[2] + ' ' + m[3])
...
// ============================================================
// PHASE 3: Fragment injection (creates burstiness)
// ============================================================
function injectFragments(sentences, strength) {
  ...
  const out = [...sentences]
  const count = strength === 'aggressive' ? Math.max(1, Math.floor(sentences.length / 2)) :
                strength === 'medium' ? Math.max(1, Math.floor(sentences.length / 3)) :
                Math.max(1, Math.floor(sentences.length / 5))

  // Insert after any sentence
  const positions = []
...
function reorderInformation(sentences, strength) {
  ...
  for (const sent of sentences) {
    ...
    // Only match if sentence starts with "It is" and doesn't contain "to note"
    if (!done && !sent.toLowerCase().includes('to note')) {
...
const FAKE_DEPTH = [
  /,\s*highlighting\s+the\s+need\s+for\s+[^.,]+/gi,
...
export function humanize(text, strength = 'medium') {
  ...
  if (cfg.split) sentences = rewriteForBurstiness(sentences, strength)
  ...
  if (cfg.vocab) {
    sentences = sentences.map(s => {
      ...
      for (const p of FAKE_DEPTH) {
        const m = s.match(p)
        if (m) {
          // Only remove if the match is small (less than 15 chars) to avoid eating content
          if (m[0].length < 15) {
            changes++
            s = s.replace(p, '').trim()
            if (!s.match(/[.!?]$/)) s += '.'
