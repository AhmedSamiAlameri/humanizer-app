...
function addImperfections(sentences, strength) {
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    if (words.length > 8) {
      ...
      positions.push({ idx: i, pos: Math.min(3, Math.floor(words.length / 3)) })
    }
  }
...
export function humanize(text, strength = 'medium') {
  ...
  // Phase 1: Filler word replacements at sentence starts
  if (cfg.fillers) {
    for (const { pattern, replacements } of FILLER_INSERTIONS) {
      const m = result.match(pattern)
      if (m) { changes++; result = result.replace(pattern, pick(replacements)) }
    }
  }
  ...
  if (cfg.fakeDepth) {
    sentences = sentences.map(s => {
      for (const p of FAKE_DEPTH) {
        const m = s.match(p)
        if (m) {
          // Only remove if match is small (< 15 chars)
          if (m[0].length < 15) {
            changes++
            s = s.replace(p, '').trim()
            if (!s.match(/[.!?]$/)) s += '.'
  ...
  // Phase 6: Add natural imperfections
